...
## Phase 6 — Remaining spec features  `[~]`
...
- **2026-07-15** — Security hardening committed+pushed (helmet, CORS allowlist, refresh-reuse detection, throttling, password policy). Then Phase 6: added `storage` (local+S3), `documents` (upload/download), and `ai` (image analysis + translation) modules — closing the last non-Flutter spec gaps. Backend now **19 modules, 35 unit tests green**, build clean, app boots with all modules. Flutter Location module (geolocator) authored + wired into hospitals/ambulance/SOS.
- **2026-07-15** — Docker recovered; ran full e2e and it **caught a real bug**: refresh rotation could mint a byte-identical token when two were issued in the same second (JWT `iat` is second-resolution), breaking reuse detection. Fixed with a per-token `jti`. Also extracted a shared `configureApp()` so main.ts and the e2e can't drift (e2e now tests the real helmet/CORS middleware) and added a documents upload/download/delete e2e. **All green: 35 unit + 17 e2e = 52 tests.**
